...
// TODO: Rename `derive_foo` once we pick a name for `Foo`.
derive!(Foo => derive_foo => derive_foo_inner);
...
// TODO: Rename once we pick a name for `Foo`.
fn derive_foo_inner(
    ast: &DeriveInput,
    _top_level: Trait,
    zerocopy_crate: &Path,
    ...
    Ok(quote!(
        const _: () = {
            ...
            #(
                unsafe impl zerocopy::init::HasField<{ #field_hashes }> for #outer_names {
                    type Type = #field_tys;
            ...
            unsafe impl init::Foo for #outer_name {
                type Uninit = (#(<#field_tys_2 as init::Foo>::Uninit,)*);
                type Init = (#(<#field_tys_3 as init::Foo>::Init,)*);
            }
        };
    ))
}
